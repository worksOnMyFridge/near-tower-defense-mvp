...
const path = Array(300).fill(0);
for (let i = 142; i <= 157; i++) path[i] = 2;
const map = {
  compressionlevel: -1,
  height: 15,
  infinite: false,
...
const outPath = path.join(__dirname, 'level_tiled.json');
require('fs').writeFileSync(outPath, JSON.stringify(map));
